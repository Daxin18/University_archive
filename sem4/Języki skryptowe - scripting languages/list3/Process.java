...
public class Process
{
    public static void main(String[] args)
    {
        ...
        int toSelect = -1; //number of a line to select from
        ...
        for(int i=0; i< args.length; i++) //petla przegladajaca argumenty i ustawiajaca wszystkie parametry
        {
            ...
            if(curr.length() >= 2 && curr.charAt(0) == '-' && curr.charAt(1) == '-')
            {
                ...
                if(curr.contains("ignorefirst") && split.length == 2 && isNumber(split[1]))
                    ...
                    sep = split[1];
                else if(curr.contains("selectFrom") && split.length == 2)
                {
                    if(isNumber(split[1]))
                    {
                        if(Integer.parseInt(split[1]) > 0)
                            toSelect = Integer.parseInt(split[1]);
                        else
                        {
                            System.out.println("\"" + split[1] + "\" is not a valid value for parameter \"selectFrom\"");
        ...
        for(String curr : text)
        {
            ...
            //select
            if(toSelect >0)//niedoprecyzowana linijka
            {
            ...
            else
            {
                ...
                for(int i = 0; i< columns.length; i++)
                    if(i == (toSelect - 1) && columns[i].equals(sel))
                        selected = true;
                //project
                if (selected)
                {
